...
3.  Чем отличаются `RWMutex` от `Mutex`?
    ...
    `Mutex` позволяет блокировать доступ к общим ресурсам с помощью методов `Lock` и `Unlock`. У `RWMutex` есть еще два метода `RLock` и `RUnlock`, которые можно использовать, если общий ресурс нуждается только в чтении, таким образом другие операции чтения не заблокируются, но операции записи заблокируются.
